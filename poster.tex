...
%}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\block{A painless $hp$-adaptive strategy}{ %
Let $\calF = \{ \phi_{i} \}_{i=1}^{n_{\calF}}$ be a set of basis functions $\phi_i$, $\H_{\calF} \coloneqq \textrm{span}\{\phi_{1}, \dots, \phi_{n_{\calF}}\}$, and $n_{\calF} = \textrm{dim}(\H_{\calF})$. For a given bilinear continuous form $b$, we consider the abstract variational formulation and its discrete version:
\innerblock{}{
\begin{quote}
...
%
%Our \enquote{painless} approach operates always on a single mesh. To do so, it requires defining a projector that simulates the presence of a coarse mesh without the trouble of handling one.
%\innerblock{}{For a given subset of basis functions $\calS\subset \calF$ that generates the space $\H_\calS \subset \H_\calF$, we define our \emph{projection operator} $\Pi_{\calF}^{\calS}$: $\H _{\calF} \longrightarrow \H_{\calS}$ as
%\begin{equation}
...
\innerblock{}{We express any $u_\calF \in \H_\calF$, as
%
\begin{equation}
  u_{\calF} =  \Pi_{\calF}^{\calE_K} u_{\calF} + \Pi_{\calF}^{\calR_K} u_{\calF},
\end{equation}
where $\calE_K \coloneqq \calF \setminus \calR_K$ is the subset of \emph{essential} basis functions.
}
...
We illustrate a 1D multi-level $hp$-grid with hierarchical basis functions proposed by Zander and collaborators.
...
%%
 ...
 %%
%%%%%%%%%%%%%%%
\block{Extension to goal-oriented adaptivity}{%These quantities are often called quantities of interest (QoIs). To control the error in the QoI, Goal-oriented adaptivity (GOA) aims to approximate specific quantities of finite element solutions. 
...
As a consequence of assuming that $\calE_K$ is (quasi) $b$-orthogonal to $\calR_K$ due to the (quasi)-orthogonality assumption of the basis functions, then
\begin{equation}
...
\end{equation}
\innerblock{}{
We define the element-wise indicators as
\begin{equation}
...
%%%%%%%%%%%%%%%%
\block{Indefinite problem}{%
Find $u$ such that, 
		\begin{alignat*}{2}
			- \nabla \cdot ( \sigma \nabla u) - k^{2}u &= \scalar{\mathds{1}_{(0,\frac{1}{4})^{d}}}{\cdot}_{L^{2}(\Omega)} \,\, && \text{ in } \Omega \subset \R^{d},\\
			u&=0 \,\, && \text{ on } {\color{blue}{\Gamma_{D}}},\\
			\grad u\cdot \vec{n} &= 0 \,\, && \text{ on } {\color{red}{\Gamma_{N}}}.
		\end{alignat*}%
\begin{align}
\SwapAboveDisplaySkip
	l(\cdot)=\scalar{\mathds{1}_{(\frac{3}{4},1)^{d}}}{\cdot}_{L^{2}(\Omega)};&&a(\cdot,\cdot) =& \, \sum_{K} \abs{\scalar{\grad \cdot}{\grad \cdot}_{L^{2}(K)}} + \abs{k^{2}} \abs{\scalar{\cdot}{\cdot}_{L^{2}(K)}}.
\end{align}
...
\begin{axis}[width=\plotwidth,height=\plotheight,
		...
		legend columns=-1
     	...
     	\addplot+[ line width=1pt] table[x expr=\thisrow{nr_dof},y expr=\thisrow{Error}] {Figures/Helm2DGOA/hp/order_1/outputs.txt}node[pos=0.9, pin={[pin edge=solid]180:$hp$}]{};
	
	\addplot+[ line width=1pt] table[x expr=\thisrow{nr_dof},y expr=\thisrow{Error}] {Figures/Helm2DGOA/h/order_1/outputs.txt}node[pos=0.9, pin={[pin edge=solid]90:$h$ ($p=1$)}]{};
	
	\addplot+[ line width=1pt] table[x expr=\thisrow{nr_dof},y expr=\thisrow{Error}] {Figures/Helm2DGOA/h/order_2/outputs.txt}node[pos=0.9, pin={[pin edge=solid]-90:$h$ ($p=2$)}]{};
...
%\legend{$\abs{l(e_h)}$,Classical bound,New bound}
     	\addplot+[ line width=1pt] table[x expr=\thisrow{nr_dof},y expr=\thisrow{Error}] {Figures/Helm3DGOA/hp/order_1/outputs.txt}node[pos=0.9, pin={[pin edge=solid]180:$hp$}]{};
...
\begin{itemize}
\item Multi-adaptive goal-oriented;
